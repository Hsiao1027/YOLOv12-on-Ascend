...
def select_device(device="", batch=0, newline=False, verbose=True):
    ...
    # ✅ 加入 NPU 支援
    if str(device).lower() == "npu":
        import torch_npu
    if not torch.npu.is_available():
        raise ValueError("NPU is not available on this system.")
    if verbose:
        LOGGER.info(f"{s}NPU (Ascend) device selected.\n")
...
def is_npu_model(model):
    # 檢查模型是否在 NPU 上（torch_npu 必須已經 import）
    try:
        import torch_npu
        for p in model.parameters():
            if hasattr(p, 'device') and 'npu' in str(p.device):
                return True
    except ImportError:
        pass
    return False
...
class ModelEMA:
    ...
    def __init__(self, model, decay=0.9999, tau=2000, updates=0):
        ...
        # 👉 NPU workaround：如果是 NPU，不執行 deepcopy
        self.enabled = not is_npu_model(model)
        self.ema = None
        ...
        if self.enabled:
            ...
            print("[ModelEMA] Disabled EMA on NPU for compatibility.")
